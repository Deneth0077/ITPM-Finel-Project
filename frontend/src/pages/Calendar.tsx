...
import { EventInput, DateSelectArg, EventClickArg, EventDropArg, EventContentArg } from "@fullcalendar/core";
...
const Calendar: React.FC = () => {
  ...
  const handleEventDrop = (dropInfo: EventDropArg) => {
    ...
    setEvents((prevEvents) =>
      prevEvents.map((ev) =>
        ev.id === event.id
          ? { ...ev, start: newStartDate, end: newEndDate }
          : ev
      )
    );
  };
      ...
      return (
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-800 dark:text-white/90">{eventDescription || "No description"}</p>
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Meal Type</label>
            <p className="text-gray-800 dark:text-white/90">{eventLevel}</p>
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Date Range</label>
            <p className="text-gray-800 dark:text-white/90">
              {eventStartDate} {eventEndDate && eventStartDate !== eventEndDate ? ` - ${eventEndDate}` : ''}
            </p>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            {Object.entries(MEAL_TYPES).map(([_, value]) => (
  ...
  return (
    <div 
      ...
      style={{
        ...
        borderLeft: `2px solid ${colors[mealType]}`,
        width: '100%',
